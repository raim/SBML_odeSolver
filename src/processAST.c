...
/** writes the given AST to standard out.  The string 'context' followed by a space
    is output before the AST
*/
...
SBML_ODESOLVER_API ASTNode_t *differentiateAST(ASTNode_t *f, char *x) {
  ...
  for ( i=0; i<List_size(list); i++ ) {
    /*printf("comparing %s with %s\n", ASTNode_getName(List_get(list, i)), x);*/
...
/**
    appends the symbols in the given AST to the given list.  'char *' strings are appended
    to the list these strings should not be freed and exist as long as the AST.
 */
...
/**
    ...
    when necessary so that the AST can be incorporated as a sub expression of another expression.
 */
...
/** appends the given node to the given buffer in compilable form assuming the node is a unary operator.
...
/** appends the given node to the given buffer in compilable form assuming the node is a Nary operator.
    'op' is the compilable operator string for the node. 
 */
...
/** appends the given node to the given buffer in compilable form assuming the node is a function.
...
/** appends compilable code to represent the given AST_Name node to the give buffer.
    The code consists of a reference to an item in the array 'value' indexed by the 
    the index associated with the node by the function 'indexAST'.  If the ASTNode
    doesn't have an index value then an error is created and '0' is appended to the buffer.
*/
...
/** appends compilable macros and functions to buffer to enable the code in buffer to support
    the code generated by the 'generateAST' function.
*/
...
/**
  appends compilable code to the given buffer for the given AST assuming the AST is an XOR expression.
*/
...
/**
  appends compilable code to the given buffer that implements the given AST.
*/
    ...
    switch (ASTNode_getType(node))
    {
        ...
        default :
            SolverError_error(FATAL_ERROR_TYPE,
                ...
                "Found stange node type whilst generating code.  Inserted '_YUCK' into code.");
