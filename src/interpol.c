...
PRIVATE int read_header_line(char *file, int n_var, char **var,
	...
	fatal(stderr, "read_first_line(): file not found");
	...
	/* exit otherwise */
	fatal(stderr, "read_first_line(): no header line found");
    }    
    /* read other columns */
    count = 0;
